...
bool visualStudioProject::createProjectFile(){
	//do the Windows folder first
	string innerDir = projectName + "/" + projectName + ".Windows/";
	string innerDirTemplate = ofFilePath::join(templatePath,"emptyExample/emptyExample.Windows/");
    string project = ofFilePath::join(projectDir,innerDir + projectName + ".Windows.vcxproj");
	...
	string filters = ofFilePath::join(projectDir, innerDir + projectName + ".Windows.vcxproj.filters");
	//string filtersXaml = ofFilePath::join(projectDir, projectName + "_winrt_xaml.vcxproj.filters");
	string tempKey = ofFilePath::join(projectDir, innerDir + projectName + ".Windows_TemporaryKey.pfx");
	string appMan = ofFilePath::join(projectDir, innerDir + "Package.appxmanifest");
	...
	ofDirectory::createDirectory(projectDir + projectName + "/");
	ofDirectory::createDirectory(projectDir + innerDir);
	ofFile::copyFromTo(innerDirTemplate+"emptyExample.Windows.vcxproj",project,false, true);
	...
	//do the WindowsPhone folder next
	innerDir = projectName + "/" + projectName + ".WindowsPhone/";
	innerDirTemplate = ofFilePath::join(templatePath,"emptyExample/emptyExample.WindowsPhone/");
    project = ofFilePath::join(projectDir,innerDir + projectName + ".WindowsPhone.vcxproj");
	filters = ofFilePath::join(projectDir, innerDir + projectName + ".WindowsPhone.vcxproj.filters");
	appMan = ofFilePath::join(projectDir, innerDir + "Package.appxmanifest");
	...
	//do the Shared folder last
	innerDir = projectName + "/" + projectName + ".Shared/";
	innerDirTemplate = ofFilePath::join(templatePath,"emptyExample/emptyExample.Shared/");
    project = ofFilePath::join(projectDir,innerDir + projectName + ".Shared.vcxitems");
	filters = ofFilePath::join(projectDir, innerDir + projectName + ".Shared.vcxitems.filters");
	...
	//assets.copyTo(projectDir + "Assets");
...
bool visualStudioProject::loadProjectFile(){
    ...
    ofFile project(projectDir + projectName + "/" + projectName + ".Shared/" + projectName + ".Shared.vcxitems");
	if(!project.exists()){
...
bool visualStudioProject::saveProjectFile(){
	...
	string filters = projectDir + projectName + "/" + projectName + ".Shared/" + projectName + ".Shared.vcxitems.filters";
	...
	bool b3 = doc.save_file((projectDir + projectName + "/" + projectName + ".Shared/" + projectName + ".Shared.vcxitems").c_str());
    return /*b0 && b1 && b2 && */b3;
}
...
void visualStudioProject::addSrc(string srcFile, string folder, SrcType type){
	...
	//needs to reference two directories up due to universal app project being two directories deep
	srcFile = "$(MSBuildThisFileDirectory)..\\..\\" + srcFile;
